...
diff --git a/src/scripts/benchmark/run_window_dynamic_mae_benchmark.py b/src/scripts/benchmark/run_window_dynamic_mae_benchmark.py
new file mode 100644
index 0000000000000000000000000000000000000000..7b5e2864f75467cb0ef754f39cc670011c089715
--- /dev/null
+++ b/src/scripts/benchmark/run_window_dynamic_mae_benchmark.py
@@ -0,0 +1,283 @@
+import argparse
+import os
+import sys
...
+        )
+        # Ensure we store a flat embedding per frame
+        if emb.ndim > 1:
+            emb = emb.mean(dim=0)
+        embeddings.append(emb)
+        confidences.append(float(spfs_info.confidence))
+
+        if len(embeddings) <= 7:
+            logits.append(0.0)
+            continue
